  ...
  app.post("/groups/:id/treasury/enable", async (req) => {
    ...
    const group = await prisma.$transaction(async (tx) => {
      ...
      await auditTx(tx, {
        userId: auth.id,
        actorType: "user",
        action: AuditAction.TREASURY_SIGNER_CHANGED,
        entityType: "group",
        entityId: id,
        metadata: {
    ...
    return runIdempotent({
      ...
      operation: async (tx) => {
        ...
        await auditTx(tx, {
          ...
          groupId: id,
          actorType: "user",
          action: AuditAction.TREASURY_DEPOSIT_CREATED,
    ...
    return runIdempotent({
      ...
      operation: async (tx) => {
        ...
        await auditTx(tx, {
          ...
          groupId: id,
          actorType: "user",
          action: AuditAction.TREASURY_WITHDRAWAL_CREATED,
          entityType: "treasury_transaction",
          entityId: ttx.id,
          metadata: {
            amount: body.amount,
      ...
      operation: async (tx) => {
        ...
        try {
          ...
          await auditTx(tx, {
            userId: auth.id,
            actorType: "user",
            action: AuditAction.TREASURY_TX_FAILED,
            entityType: "treasury_transaction",
            entityId: id,
            outcome: "failure",
            metadata: {
        ...
        await auditTx(tx, {
          userId: auth.id,
          actorType: "user",
          action: AuditAction.TREASURY_TX_CONFIRMED,
          entityType: "treasury_transaction",
          entityId: id,
